...
| RK-07 | Fiscal reports may double-count NIR↔invoice | S1→S2 | derived from doc lists | **Partially mitigated**: the persisted journal respects the 3-way match (a factura_cumparare linked to a posted NIR generates no second note). Fiscal declarations (D300/D394/D390) still derive from documents — fiscal-event layer (P7) | partially mitigated |
