...
| RK-03 | Stock/accounting derived, not persisted → not immutable/auditable | S1→S3 | was no ledger tables | **Downgraded (stock)**: persistent append-only `stock_ledger_entries` + materialized `stock_balances`, emitted atomically at posting; reports derive from it. Accounting ledger still derived — Phase 6 | downgraded |
